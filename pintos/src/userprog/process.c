...
process_execute (const char *file_name)
{
    ...
    sema_down(&thread_current()->exec_semaphore);
    if (!thread_current()->execute_success) tid = TID_ERROR;
    return tid;
}
...
start_process (void *file_name_)
{
    ...
    thread_current()->parent_thread->execute_success = true;
    sema_up(&thread_current()->parent_thread->exec_semaphore);
...
process_wait (tid_t child_tid UNUSED)
{
    ...
    struct child_thread *act = NULL;

    //find the child
    for (e = list_begin (l); e != list_end (l); e = list_next (e))
    {
        act = list_entry (e, struct child_thread, child_thread_elem);
